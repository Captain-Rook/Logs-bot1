...
bot.on('message', (message) => {
    ...
    const warning_A_msg = ["даун", "довн", "додик", "dayn", "аут", "аутист", "мразь", "мразота", "тварь", "тварина", " конч ", "конченый", "подонок",
     "чурка", "членосос", "гнида", "ублюдок", "с1ка", "ебл1н", "нах1й", "nahui", "nahyi", "naxui", "манда", "шмара",];
     const dateChat = moment.tz(timezone).format('DD.MM.YYYY HH:mm:ss');
     if (warning_A_msg.some(str => msg.toLowerCase().includes(str))) {
        telegramBot.sendMessage(chatId, `[${dateChat}] ${message}`);                       // сообщение в телеграм при сообщении с использованием слова/слов из чс
    }
});
...
function checkMessage(player, message) {
    ...
    const date = moment.tz(timezone).format('DD.MM.YYYY HH:mm:ss');
    if (players[player] >= 3) {
    telegramBot.sendMessage(5230010894, `[${date}] ${player} отправил 3 плохих слова`);
      players[player] = 0;
    }
}
    ...
    if (!lastMessages[username]) {
      ...
      && now - lastMessages[username].timestamp < similarMessagesInterval) {
        ...
        if (lastMessages[username].count >= similarMessagesThreshold) {
             const date = moment.tz(timezone).format('DD.MM.YYYY HH:mm:ss');
             const telegramMessage = `Игрок ${username} спамит\nОтправка ${lastMessages[username].count} похожих между собой сообщений в течении 2 минуты.`;
